  ...
  const Content = () => {
    return (
      <div className={`${path === 4 ? 'flex-col' : 'flex-row'} flex pb-5`}>
        <div
          className={`${
            path === 4 ? 'w-full items-center' : 'w-1/12'
          } flex space-x-3 pt-1`}
        >
        ...
        >
          {path !== 4 && (
            <div className='flex w-11/12 xl:w-full'>
              <h1 className='font-medium dark:text-white'>
                {UserVerificateRow({ name, username, range })}
                {comment && <TimeAgo datetime={createdAt} locale='es' />}
              </h1>
          ...
          <div className='flex flex-col w-11/12 mt-1 xl:w-full'>
            {content
              ? (
                <p className='w-full flex mb-2 text-lg font-[450]  text-white'>
                  {content.split(' ').map((v) => (
                    <>
                      <span
                        className={`${
                        v.includes('#') ? 'text-blue1' : 'mr-[5px]'
                      }`}
                      >
                        {v}
                      </span>
